...
(assertion strength, determinism, scope) and a **verification pass** (clean
...
**Exit:** CI green from a clean checkout; the verification pass confirms toxics
...
**Exit:** suite green x3 consecutive runs (concurrency-sensitive tests);
...
- `README.md` for portfolio readers: the architecture, one worked example
  ...
  command.
- Flake audit: full suite x5 from clean; any flake is fixed or the test is
  redesigned - no retries policy for invariant assertions.

**Exit:** CI fully green from scratch on a fresh clone; both review gates
